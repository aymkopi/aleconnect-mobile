...
import { consumerAccountLinkRequestStatusLabel } from "@/features/accounts/status";
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, gap: 14 }}>
        ...
        {requests.length ? <View className="gap-3 pt-2"><Heading size="md">Link requests</Heading>{requests.map((request) => <View key={request.requestId} className="gap-1 rounded-lg border border-border bg-card p-3"><Text className="font-bold">{request.accountNumber} · {request.registeredName}</Text><Text className="text-sm text-muted">{consumerAccountLinkRequestStatusLabel(request.status)}</Text>{request.status === "denied" && request.consumerReason ? <Text className="text-sm text-destructive">{request.consumerReason}</Text> : null}{request.status === "approved" ? <><Text className="text-sm text-muted">Approved. Sign out and continue with email to refresh account access.</Text><Button onPress={() => void signOut().then(() => router.replace({ pathname: "/sign-in", params: { mode: "email", linked: "1" } }))} size="sm"><ButtonText>Continue with email</ButtonText></Button></> : null}{request.status === "conflict" ? <Text className="text-sm text-muted">This request needs additional staff review.</Text> : null}</View>)}</View> : null}
