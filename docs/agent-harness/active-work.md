...
## Compact human-reference rollout physical compatibility evidence (2026-08-30)

- Canonical source remains `master` at `3813d49a176afc5bac58fce5af31c8c8185ed9b9`; no Mobile source/config files changed in this evidence pass.
- One physical Android target `192.168.1.31:39101` (`25040RP0AG`) was selected. `com.kapecakes.aleconnectmobile` remained version `1.0.0`/code `1`, target SDK 36, with `firstInstallTime=2026-08-06 08:03:24` unchanged after the local debug upgrade and a valid data directory/path.
- `npx expo run:android --device 25040RP0AG --no-bundler` completed `BUILD SUCCESSFUL in 4m 6s`; local debug APK SHA-256 was `A5C0E15DCDD49C39089712E49F685CF0977848AA35FAB6686E0D7CC6F3E5`. Guest Home, public Hotlines, the sign-in gate, and restart rendering were physically observed; immediate crash-focused logcat was empty.
- Authenticated sign-in/logout, compact/legacy human-reference rendering, UUID notification routing, report submission/replay/history, offline queue refresh, and production release/adoption remain unverified. No EAS build/submission, store publication, backend/Cloudflare/DB operation, backup, manifest, or compact cutover occurred.
## Advisory date/publication policy implementation (2026-08-31)

- Scope: coordinated consumer readers preserve the nullable advisory API/cache contract while normalizing older cached records and presenting restoration and end-only timing correctly. General/weather interruption lines remain hidden; no required fields were added.
